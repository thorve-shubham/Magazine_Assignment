...
def combined_search(db: Session, query: str, page: int = 1, page_size: int = 10, min_score: float = 0.15):
    try:
        ...
        query_vector = get_embeddings(query) # This should return a vector, a 768-dimensional list

        query_vector = np.array(query_vector, dtype=np.float32)

        # convert np.array float32 to string as we will case this to vector in query
